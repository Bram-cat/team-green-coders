...
import { PEI_SOLAR_COMPANIES } from '@/lib/data/peiSolarData'
import { X, CheckCircle2, Mail, Phone, User, MessageSquare } from 'lucide-react'
...
export function QuoteRequestModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Failed to submit quote request. Please try again or contact companies directly.')
    } finally {
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Get free quotes from certified PEI solar installers based on your analysis
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            <div className="py-16 text-center space-y-6 animate-in zoom-in-95 duration-300">
              ...
              <div>
                ...
                <p className="text-muted-foreground max-w-md mx-auto">
                  Your analysis has been sent to {selectedCompanies.length} {selectedCompanies.length === 1 ? 'company' : 'companies'}.
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {analysisData && (
                <div className="bg-muted/30 rounded-lg p-4 space-y-2">
                  <h3 className="font-semibold text-foreground text-sm">Your Analysis Summary</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div>
                      <div className="text-muted-foreground">System Size</div>
                      <div className="font-bold text-foreground">{analysisData.systemSizeKW} kW</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Panels</div>
                      <div className="font-bold text-foreground">{analysisData.panelCount}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Annual Production</div>
                      <div className="font-bold text-foreground">{analysisData.annualProductionKWh.toLocaleString()} kWh</div>
                    </div>
                    {analysisData.estimatedCost && (
                      <div>
                        <div className="text-muted-foreground">Est. Cost</div>
                        <div className="font-bold text-foreground">${analysisData.estimatedCost.toLocaleString()}</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="text-base font-semibold">Select Companies to Contact</Label>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {selectedCompanies.length === PEI_SOLAR_COMPANIES.length ? 'Deselect All' : 'Select All'}
                  </Button>
                </div>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground mt-1">{company.location}</div>
                          {company.specialties && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {company.specialties.slice(0, 2).map((specialty, i) => (
                                <span
                                  key={i}
                ...
                <p className="text-xs text-muted-foreground">
                  Selected: {selectedCompanies.length} of {PEI_SOLAR_COMPANIES.length} companies
                </p>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-4 text-xs text-blue-800 dark:text-blue-300">
                <p className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>Privacy Notice:</strong> Your contact information and analysis results will only be shared with the companies you select.
                    SolarPEI does not sell or share your data with third parties.
                  </span>
                </p>
              </div>
